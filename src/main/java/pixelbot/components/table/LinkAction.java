...
public class LinkAction extends AbstractHyperlinkAction<Link> {
	private static final long serialVersionUID = 1349885063871594588L;

	private static final Logger LOG = Logger.getLogger(LinkAction.class.getName());

	private Action desktopAction;
	private URIVisitor visitor;

	/**
	 * Factory method to create and return a HyperlinkAction for the given uri. Tries to guess the
	 * appropriate type from the uri. If uri is not null and has a scheme of mailto, create one of
	 * type Mail. In all other cases, creates one for BROWSE.
	 * 
	 ...
	 */
	public static LinkAction createHyperlinkAction(Link link) {
		Action type = isMailURI(link.getURI()) ? Action.MAIL : Action.BROWSE;
		return createHyperlinkAction(link, type);
	}
	 ...
	 * @param desktopAction the type of desktop action this class should perform, must be BROWSE or
	 * MAIL
	 ...
	 */
	public static LinkAction createHyperlinkAction(Link link, Action type) {
		return new LinkAction(link, type);
	}
	 ...
	 * @throws IllegalArgumentException if unsupported action type
	 */
	public LinkAction(Action desktopAction) {
	 ...
	 * @throws IllegalArgumentException if unsupported action type
	 */
	public LinkAction(Link uri, Action desktopAction) {
		super();
	 ...
	 * <p>
	 * 
	 * Implemented to perform the appropriate Desktop action if supported on the current target.
	 * Sets the visited property to true if the desktop action doesn't throw an exception or to
	 * false if it did.
	 * 
	...
	public void actionPerformed(ActionEvent e) {
		if (!getURIVisitor().isEnabled(getTarget().getURI()))
			return;
		try {
			getURIVisitor().visit(getTarget().getURI());
			setVisited(true);
	...
	private void updateEnabled() {
		setEnabled(getURIVisitor().isEnabled(getTarget() == null ? null : getTarget().getURI()));
	}
	...
	/**
	 * Thin wrapper around Desktop functionality to allow uniform handling of different actions in
	 * HyperlinkAction.
	 * 
	 */
	protected abstract class URIVisitor {
		protected boolean desktopSupported = Desktop.isDesktopSupported();

		/**
		 * Returns a boolean indicating whether the action is supported on the given URI. This
		 * implementation returns true if both the Desktop is generally supported and
		 * <code>isActionSupported()</code>.
		 * 
		 * PENDING JW: hmm ... which class exactly has to check for valid combination of Action and
		 * URI?
		 * 
		 * @param uri
		 * @return
		 ...
		 * 
		 * @return true if the Action is supported by the current desktop, false otherwise.
		 */
		protected abstract boolean isActionSupported();
	}

	protected class BrowseVisitor extends URIVisitor {
	...
	protected class MailVisitor extends URIVisitor {
